...
public class DecayedGrassAndFireflyBlock extends TallPlantBlock {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack){
        ...
        double pos_x = pos.getX() + 0.5;
        double pos_y = pos.getY() + 0.5 - 1;
        double pos_z = pos.getZ() + 0.5;
        BlockPos replaceBlockPos = combineBlockPos(pos_x, pos_y, pos_z);

        world.setBlockState(replaceBlockPos, Blocks.PODZOL.getDefaultState());
    }
    /**
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        float y_offset = random.nextFloat() * 6;
        ...
        double pos_y_particle = pos.getY() + 0.5 + y_offset;
        double pos_z = pos.getZ() + 0.5 - 2 + z_offset;
        int randomCheck = random.nextInt(25);//概率判断器
        ...
        BlockPos particlePos = combineBlockPos(pos_x, pos_y_particle, pos_z);

        if(world.getTimeOfDay() >= 13000){
            world.addParticle(StatementsAndRegisters.FIREFLY, pos_x, pos_y_particle - 2, pos_z, random.nextDouble() * 0.05, random.nextDouble() * 0.05, random.nextDouble() * 0.05);
        }
        ...
        if (world.getBlockState(pos).getBlock() == StatementsAndRegisters.DECAYED_GRASS_AND_FIREFLY && world.getBlockState(nowBlockPos).isIn(BlockTags.CROPS)) {
            ...
            if(world.getBlockState(nowBlockPos).contains(Properties.AGE_3)){//第一个分支，判定 nowBlockPos 处的方块是否拥有 AGE_3 标签

                int i = world.getBlockState(nowBlockPos).get(Properties.AGE_3);//获取当前位置 crop 方块的 age 值

                if (i < 3) {//限制条件 1, 保证当前 age 值一定小于 3，否则会产生溢出
                    if (world.getBlockState(beneathBlockPos).getBlock() == Blocks.FARMLAND && world.getBlockState(beneathBlockPos).get(FarmlandBlock.MOISTURE) > 0 && randomCheck == 0) {
                        //上为限制条件 2, 判定当前位置 crop 方块下方一个方块处是否为 Farmland 且是否已经润湿，最后一个条件使得本 if 条件句有 0.04 的概率执行成功
                        world.setBlockState(nowBlockPos, world.getBlockState(nowBlockPos).with(Properties.AGE_3, i + 1));//使在 nowBlockPos 处的作物方块 age 值 +1

                        for(int j = 0; j < 10; j++){//循环执行使粒子效果的量增多
                            world.addParticle(ParticleTypes.COMPOSTER, pos_x, pos_y, pos_z, 0.0, 0.05, 0.0);//生成粒子效果，COMPOSTER 为堆肥桶堆肥成功的粒子
                        }
                    }
                }
            } else if (world.getBlockState(nowBlockPos).contains(Properties.AGE_7)) {//第二个分支，判定 nowBlockPos 处的方块是否拥有 AGE_7 标签
                //以下与上一个分支相同，除去数据更改
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, @NotNull BlockPos pos) {
        BlockPos blockPos = pos.down();//获取方块的下平面
        return this.canPlantOnTop(world.getBlockState(blockPos), world, blockPos);//调用 canPlantOnTop 方法，设置允许放置在什么方块上
    }

    protected boolean canPlantOnTop(@NotNull BlockState floor, BlockView world, BlockPos pos) {
        return floor.isIn(BlockTags.DIRT) || floor.isOf(Blocks.FARMLAND);//允许放置在 tag 为 dirt 的方块与 farmland 上（虽然放置后会使农田变成泥土）
    }
}
